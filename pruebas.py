...
def mostrarRegistros(FD):
    if not os.path.exists(FD):
        ...
        prueba = open("prueba.csv", "wt")
        size = os.path.getsize(FD)

        while registros.tell() < size:
            registro = pickle.load(registros)
            campo = (str(registro.codigo) + "," +str(registro.patente) + "," + str(registro.tipoV) + "," + str(registro.forma_de_pago) + "," + str(registro.pais) + "," + str(registro.km_Recorridos) + "\n")
            prueba.write(campo)
        registros.close()
